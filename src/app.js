...
async function persist(message = '') {
  ...
  saveInFlight = saveInFlight.catch(() => {}).then(async () => {
    ...
    try {
      ...
      storageWarning = folderResult === 'permission-needed' ? 'Your change is saved in this browser, but the Vault folder needs permission before it can be updated.' : '';
    } catch (error) {
      storageWarning = `Your change is saved in this browser, but the Vault folder was not updated: ${error instanceof Error ? error.message : 'folder write failed'}`;
    }
  });
  persistenceDepth += 1;
  try {
...
function renderShell() {
  ...
  app.innerHTML = `
    <header class="app-header">
      ...
      <span class="header-create-group" role="group" aria-label="Items and Tags"><button class="primary" data-action="create-root" aria-label="Add item">${icon('bag')}<span class="header-label">Item</span></button><button data-action="tags" title="Tags">${icon('tag')}<span class="header-label">Tags</span></button></span>
...
function renderSources() {
  ...
  const destination = parentId ? state.entities[parentId]?.name : 'Vault root';
  const sourceCard = source => `<article class="source-card"><button class="source-choice" data-action="choose-source" data-id="${source.id}">${source.image ? `<img class="source-mark" src="${escape(source.image)}" alt="">` : `<span class="source-mark">${escape(source.name.slice(0, 1).toUpperCase())}</span>`}<span><strong>${escape(source.name)}</strong><small>${escape(source.description)}</small></span></button><button class="icon-button" data-action="edit-source" data-id="${source.id}" aria-label="Edit ${escape(source.name)} source">${icon('more')}</button></article>`;
  ...
  return `<section class="panel active" aria-labelledby="sources-title">${utilityHeader('Add to inventory', `Destination: ${destination}`)}<div class="panel-body"><div class="source-heading"><div><span class="eyebrow">Item sources</span><h3>What are you adding?</h3><p class="muted">Tiles are editable data. Reorder, hide, rename, re-image, or replace them.</p></div></div><div class="source-grid">${visible.map(sourceCard).join('')}<article class="source-card add-source-card"><button class="source-choice" data-action="new-source"><span class="source-mark">+</span><span><strong>New source</strong><small>Add another reviewed behavior or saved query.</small></span></button></article></div>${hidden.length ? `<details class="hidden-sources"><summary>${hidden.length} hidden ${hidden.length === 1 ? 'source' : 'sources'}</summary><div class="source-grid">${hidden.map(sourceCard).join('')}</div></details>` : ''}</div></section>`;
}
...
function renderSourceEditor() {
  ...
  return `<section class="panel active" aria-labelledby="source-editor-title">${utilityHeader(source ? 'Edit item source' : 'New item source', 'Configure supported behavior without scripting')}<form class="panel-body" data-form="source-editor"><label>Name<input name="name" required maxlength="80" value="${escape(source?.name || '')}"></label><label>Description<textarea name="description" maxlength="300">${escape(source?.description || '')}</textarea></label><label>Behavior<select name="behavior">${[['create-item','Create an item'],['create-container','Create a Container'],['create-character','Create a Character Container'],['custom-create','Choose Item, Container, or Character'],['browse-query','Browse a saved query'],['dnd-tools','D&D tools'],['ddb-import','D&D Beyond PDF import']].map(([value, label]) => `<option value="${value}" ${source?.behavior === value ? 'selected' : ''}>${label}</option>`).join('')}</select></label><label>Query<span class="query-input-shell"><span class="query-highlight" aria-hidden="true">${renderQueryHighlight(queryDraft)}</span><input class="query-input" name="query" value="${escape(queryDraft)}" placeholder="+Managed" data-action="collection-query" role="combobox" autocomplete="off" spellcheck="false" aria-autocomplete="list" aria-controls="collection-suggestions" aria-expanded="${Boolean(activeQueryOperand(queryDraft))}"></span><span>Used by Browse behavior and as the D&D item filter.</span></label><div id="collection-query-assist">${renderQueryAssist(queryDraft, 'collection')}</div><div class="editor-image-row">${imagePicker('image', source, source?.image ? 'Change tile image' : 'Add tile image')}<span>Optional square image shown on the source tile.</span></div><label class="choice"><input name="enabled" type="checkbox" ${source?.enabled === false ? '' : 'checked'}><strong>Show this source</strong><span>Hidden sources remain editable from the collapsed list.</span></label>${source ? `<div class="form-actions source-order"><button class="square-button" type="button" data-action="source-move" data-id="${source.id}" data-direction="-1" ${index <= 0 ? 'disabled' : ''} aria-label="Move up" title="Move up">${icon('up')}</button><button class="square-button" type="button" data-action="source-move" data-id="${source.id}" data-direction="1" ${index >= state.itemSources.length - 1 ? 'disabled' : ''} aria-label="Move down" title="Move down">${icon('down')}</button></div>` : ''}<p class="muted">Only reviewed behaviors are available. Source data never runs user JavaScript.</p><div class="form-actions">${source ? `<button class="danger" type="button" data-action="delete-source" data-id="${source.id}">Delete</button>` : ''}<button type="button" data-action="sources-back">Cancel</button><button class="primary" type="submit">Save</button></div></form></section>`;
}
...
function prepareDdbImport(result, targetId = '') {
  ...
  const automaticTags = [itemTag?.id, createdTag?.id].filter(Boolean);
...
function renderEditor() {
  const context = /** @type {{id?:string,parentId?:string,kind?:string,presetTagName?:string,presetTagNames?:string[]}} */(utilityContext);
  ...
  const selectedTagIds = new Set(tags.filter(tag => existing?.tags.includes(tag.id) || presetNames.has(tag.name)).map(tag => tag.id));
  ...
  if (isTag) {
    ...
    return `<section class="panel active" aria-labelledby="editor-title">${utilityHeader(isCreate ? 'Create Tag' : 'Tag Settings', 'Reusable label and metadata')}
      ...
      <div class="form-actions">${existing ? `<button class="danger" type="button" data-action="request-delete-entity" data-id="${existing.id}">Delete</button><button class="danger purge-button" type="button" data-action="request-purge-tag" data-id="${existing.id}">Purge</button>` : ''}<button type="button" data-action="${utilityDismissAction()}">Cancel</button><button class="primary" type="submit">${isCreate ? 'Create' : 'Save'}</button></div></form>${confirmingPurge ? `<div class="panel-confirm-layer" role="alertdialog" aria-modal="true" aria-labelledby="purge-title"><div class="confirm-dialog"><span class="eyebrow">Permanent cleanup</span><h3 id="purge-title">Purge ${escape(existing.name)}?</h3><p>This deletes the Tag and ${directUses} ${directUses === 1 ? 'thing' : 'things'} currently using it. Deleted Containers also hide everything inside them. Undo restores the full change.</p><div class="form-actions"><button data-action="cancel-purge-tag">Cancel</button><button class="danger" data-action="purge-tag" data-id="${existing.id}">Purge ${directUses + 1}</button></div></div></div>` : ''}</section>`;
  }
...
async function handleClick(target) {
  ...
  if (action === 'new-source') openUtility('source-editor', { parentId: /** @type {any} */(utilityContext).parentId || '' });
  ...
  if (action === 'choose-source') {
    ...
    if (!source) return;
    const parentId = /** @type {any} */(utilityContext).parentId || '';
    if (source.behavior === 'create-item') openUtility('editor', { parentId, kind: 'item', presetTagNames: source.presetTagNames || [] });
    if (source.behavior === 'create-container') openUtility('editor', { parentId, kind: 'container' });
    if (source.behavior === 'create-character') openUtility('character-setup', { parentId });
    if (source.behavior === 'custom-create') openUtility('custom-create', { parentId });
    if (source.behavior === 'browse-query') openUtility('source-browser', { parentId, query: source.query, queryBindings: source.queryBindings || [], name: source.name });
    if (source.behavior === 'dnd-tools') openUtility('dnd-tools', { parentId, query: source.query || '+D&D5e', queryBindings: source.queryBindings || [], name: source.name });
    if (source.behavior === 'ddb-import') openUtility('ddb-import', { parentId, mode: 'new' });
  }
  if (action === 'custom-item') openUtility('editor', { parentId: /** @type {any} */(utilityContext).parentId || '', kind: 'item', presetTagNames: target.dataset.unique ? ['Unique'] : [] });
  if (action === 'custom-container') openUtility('editor', { parentId: /** @type {any} */(utilityContext).parentId || '', kind: 'container' });
  if (action === 'custom-character') openUtility('character-setup', { parentId: /** @type {any} */(utilityContext).parentId || '' });
  if (action === 'custom-tag') openUtility('editor', { kind: 'tag' });
  if (action === 'dnd-browse') openUtility('source-browser', { parentId: /** @type {any} */(utilityContext).parentId || '', query: /** @type {any} */(utilityContext).query || '+D&D5e', queryBindings: /** @type {any} */(utilityContext).queryBindings || [], name: 'D&D items' });
  if (action === 'blank-character') openUtility('editor', { parentId: /** @type {any} */(utilityContext).parentId || '', kind: 'container', presetTagName: 'Character' });
  if (action === 'ddb-new') openUtility('ddb-import', { parentId: /** @type {any} */(utilityContext).parentId || '', mode: 'new' });
  if (action === 'ddb-update') openUtility('ddb-import', { parentId: /** @type {any} */(utilityContext).parentId || '', mode: 'update' });
  ...
  if (action === 'copy-source-item') {
    ...
    const copy = { ...structuredClone(sourceEntity), id, parentId, quantity: 1, tags: [...new Set([...sourceEntity.tags.filter(tagId => tagName(tagId) !== 'Managed'), ...automaticTags])], managed: false, createdAt: now, updatedAt: now };
